...
QuestionList CreateHomophoneExam()
{
	...
	} data[] = 
	{
		...
		{"ほしょう",{
			...
			{ "保障", "権利や地位などが維持されるように保護し護ること" },
			{ "補償", "損失をおぎなってつぐなうこと" }}},
		{"たいしょう", {
			{ "対象", "行為の目標となるもの" },
			{ "対称", "2つの図形た物事が互いにつり合っていること" },
			{ "対照", "見比べること、違いが際立つこと" }}},
		{"あやまる", {
			{ "謝る", "失敗について許しを求める" },
			{ "誤る", "間違った判断をする" }}},
	...
	for (int i = 0; i < quizCount; i++)
	{
		...
		//要素数を計算
		int count = 0; for (; count < size(e.words); count++)
		{
			if (!e.words[count].kanji)	break;
		}
	...
	for (int i = 0; i < quizCount; i++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			if (j == correctNo - 1)
			{
				...
				if (other >= object)	other++;  //出題する語の番号を飛ばす
