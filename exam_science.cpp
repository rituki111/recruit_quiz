...
QuestionList CreatePhysicsExam()
{
	...
	{  //等加速度直線運動
		//公式: v = v0 + at を使って解く問題
		int v0 = uniform_int_distribution<>(1, 10)(rd);  //加速
	...
	{  //重力加速度
		...
		string answer = to_string(x / 10);  //整数部分を文字列化
		if (x % 10)
		{
			//小数部がある場合は小数部を文字列化
			answer += '.';
		...
		questions.push_back({
			...
			"秒後の物体の速度はxm/sである。\n" +
			"xの値を小数点以下第1位なで求めよ(空気抵抗はないものとする)。",
			answer });
	}  //重力加速度
	...
	{  //浮力
		...
		if (v % 10)  //小数点以下第1位が0以外なら、小数部を文字列に加える
		{
			answer += '.';
			answer += '0' + v % 10;
		}
	...
	{  //ばね
		...
		questions.push_back({
			...
			"の2角ばねを直列につなぎ、" + to_string(m) + "gの重りをつけて天井から吊り下げた。\n" +
			"すると、ばねの長さが合わせてXcm伸びて静止した。\n" +
			"Nの値を小数点以下第2位を四捨五入して求めよ。",
			answer });
		...
		questions.push_back({
			"重力加速度を10m/s^2とする。\nばね定数が" + to_string(k1) + "と" + to_string(k2) +
			"の2本のばねを水平な天井に固定し、ばねの下端に木の棒を水平に取り付けた。\n" +
	...
	{  //振り子
		...
		questions.push_back({
			...
			"このとき、重りが最下点を通過する時の速度をm/s単位で求めよ。",
			to_string(v) });
	}  //振り子
